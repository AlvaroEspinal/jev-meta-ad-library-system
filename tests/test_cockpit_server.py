...
class CockpitServerTests(unittest.TestCase):
    def setUp(self):
        ...
        else: raise RuntimeError(self.process.stderr.read())
    def tearDown(self):
    ...
    def post(self, event, token=TOKEN):
        request=Request(self.url('/api/events'),data=json.dumps(event).encode(),method='POST',headers={'Content-Type':'application/json','Authorization':f'Bearer {token}'})
